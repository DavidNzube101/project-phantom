<script>
	// SvelteKit Imports for URL management
	...
	const mockResults = [
		{
			...
			description:
				"Learn essential privacy practices, tools, and techniques to protect your digital footprint. Comprehensive coverage of encryption, secure communications, and data protection.",
			timestamp: "2h ago",
			verified: true,
			hasLogo: true,
		},
		{
			...
			description:
				"Discover powerful open-source alternatives to mainstream apps that respect your privacy. From browsers to messaging apps and beyond.",
			timestamp: "4h ago",
			verified: true,
			hasLogo: false,
		},
		{
			...
			description:
				"A deep dive into how companies collect your data, what rights you have, and practical steps to limit unwanted data harvesting.",
			timestamp: "1d ago",
			verified: false,
			hasLogo: true,
		},
		{
			...
			description:
				"Comprehensive review of the most trusted VPN services that prioritize user privacy and security. Compare features, pricing, and performance.",
			timestamp: "3h ago",
			verified: true,
			hasLogo: false,
		},
		{
			...
			description:
				"Compare the security features of popular encrypted messaging apps. Learn about end-to-end encryption, metadata protection, and more.",
			timestamp: "5h ago",
			verified: true,
			hasLogo: true,
		},
	...
	/**
	 * Updates the URL with the current search query.
	 * This function is called by user actions (e.g., clicking the search button).
	 * The actual search logic is triggered by the $effect that watches the URL.
	 */
	function handleSearch() {
		if (!searchQuery.trim()) return;

		executeSearch(); // Call executeSearch directly

		const newUrl = new URL($page.url);
		newUrl.searchParams.set("q", encodeURIComponent(searchQuery));

		// Navigate to the new URL. This will trigger the $effect to run the search.
		// We only navigate if the URL is different to avoid pushing duplicate history entries.
		if (newUrl.href !== $page.url.href) {
			goto(newUrl.href, { noScroll: true, replaceState: false });
		}
	}

	/**
	 * Executes the search itself. This is separated so it can be called
	 * when the URL changes, making the URL the source of truth.
	 */
	...
	function detectSearchType(query) {
		...
		const currencyKeywords = ["usd", "eur", "gbp", "jpy", "btc", "eth", "sol", "exchange", "currency", "convert", "to"];
		if (currencyKeywords.some((k) => lowerQuery.includes(k))) {
			showCurrencyExchange = true;
		}

		const weatherKeywords = ["weather", "temperature", "forecast", "rain", "sunny", "cloudy", "snow"];
		if (weatherKeywords.some((k) => lowerQuery.includes(k))) {
			showWeather = true;
		}

		const tokenChartKeywords = [
			{ keyword: "sol price", symbol: "SOL" }, { keyword: "sol chart", symbol: "SOL" },
			{ keyword: "btc price", symbol: "BTC" }, { keyword: "btc chart", symbol: "BTC" },
			{ keyword: "eth price", symbol: "ETH" }, { keyword: "eth chart", symbol: "ETH" },
		];
		...
		const questionPatterns = [
			{ pattern: /what is (.*)/i, type: "definition" }, { pattern: /how to (.*)/i, type: "howto" },
			{ pattern: /when (.*)/i, type: "when" }, { pattern: /where (.*)/i, type: "where" },
	...
	function exportResults(format) {
		...
		if (format === "pdf") {
			...
			const csvContent = ["Title,URL,Description,Timestamp,Verified", ...mockResults.map((r) => `"${r.title}","${r.url}","${r.description}","${r.timestamp}","${r.verified}"`).join("\n")]
	...
	$effect(() => {
		// This effect synchronizes the URL with the component's state and initializes browser APIs.

		// Part 1: Sync URL search query with component state
		const urlQuery = $page.url.searchParams.get("q");
		const decodedUrlQuery = urlQuery ? decodeURIComponent(urlQuery) : null;

		if (decodedUrlQuery && decodedUrlQuery !== searchQuery) {
			// If URL has a query that differs from the state, update state and search
			searchQuery = decodedUrlQuery;
			executeSearch();
		} else if (!decodedUrlQuery && hasSearched) {
			// If URL has no query but we're in a searched state (e.g., user clicks 'back'), reset the UI
			searchQuery = "";
			hasSearched = false;
			// Reset all conditional widgets
			showCurrencyExchange = false;
			showWeather = false;
			showSpecificAnswer = false;
			showSolanaExplorer = false;
		...
		// Part 2: Initialize browser-only Speech Recognition API
		if (typeof window !== "undefined" && !recognition) {
			if ("webkitSpeechRecognition" in window || "SpeechRecognition" in window) {
				...
				newRecognition.onstart = () => { isListening = true; };
				newRecognition.onerror = () => { isListening = false; };
				newRecognition.onend = () => { isListening = false; };

				newRecognition.onresult = (event) => {
					const transcript = event.results[0][0].transcript;
					searchQuery = transcript;
					isListening = false;
					handleSearch(); // Trigger a search, which will update the URL
				};
...
<div
	class="min-h-screen bg-gradient-to-b from-purple-950 via-purple-900 to-slate-950"
>
	<!-- Header -->
	<header
		class="sticky top-0 z-50 border-b border-purple-800/30 bg-purple-950/80 backdrop-blur-xl"
	>
		<div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
			<div class="flex h-16 items-center justify-between">
				<a href="https://breeze.appwrite.network">
					<div class="flex items-center gap-3 cursor-pointer">
						<!-- <div
							class="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-purple-500 to-purple-700 shadow-lg shadow-purple-500/25"
						> -->
						<div
							class="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br"
						>
							<img src="/favico.png" alt="Breeze Search Logo" class="h-10 w-10"/>
						</div>
						<div>
				...
				<div class="flex items-center gap-4">
					<button
						onclick={() =>
							(showPrivacyFeatures = !showPrivacyFeatures)}
			...
			<div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-4">
				<div class="grid gap-6 sm:grid-cols-3">
					<div class="flex items-center gap-3">
						<div
							class="flex h-8 w-8 items-center justify-center rounded-lg bg-purple-800/50"
						>
	...
	<main class="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
		{#if !hasSearched}
			...
			<div class="py-20 text-center">
				<!-- <div
					class="mx-auto mb-8 flex h-20 w-20 items-center justify-center rounded-2xl bg-gradient-to-br from-purple-500 to-purple-700 shadow-2xl shadow-purple-500/25"
				> -->
				<div
					class="mx-auto flex h-20 w-20 items-center justify-center rounded-2xl bg-gradient-to-br"
				>
					<img src="/favico.png" alt="Breeze Search Logo" class="h-20 w-20"/>
				</div>
				<!-- <h1
					class="mb-4 text-5xl font-bold tracking-tight text-white sm:text-6xl"
				>
					Search with
					<span
				...
				<p
					class="mx-auto mb-12 max-w-2xl text-xl text-purple-200 leading-relaxed"
				>
					Experience the web without compromise. No tracking, no ads,
					no data collection. Just pure, private search results.
				</p> -->
			</div>
		{:else}
			<div class="py-8"></div>
			...
			<div class="relative">
				<div class="flex gap-3">
					<div class="relative flex-1">
						...
						{#if showSuggestions}
							<div
								class="absolute top-full left-0 right-0 mt-2 bg-purple-900/90 border border-purple-700/50 rounded-xl shadow-lg backdrop-blur-sm z-50"
							>
		...
		{#if !hasSearched}
			...
			<!-- <div class="grid gap-8 lg:grid-cols-2">
				...
				>
					<CardContent class="p-6">
						<div class="mb-6 flex items-center gap-3">
							<div
								class="flex h-8 w-8 items-center justify-center rounded-lg bg-orange-500/20"
							>
				...
				>
					<CardContent class="p-6">
						<div class="mb-6 flex items-center gap-3">
							<div
								class="flex h-8 w-8 items-center justify-center rounded-lg bg-blue-500/20"
							>
			...
			<!-- Privacy Promise -->
			<div
				class="mt-16 rounded-2xl bg-gradient-to-r from-purple-900/30 to-purple-800/30 backdrop-blur-sm p-8 text-center border border-purple-700/30"
			>
				<div
					class="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-xl bg-gradient-to-br from-purple-500 to-purple-700"
				>
				...
				<p class="mx-auto max-w-2xl text-purple-200">
					We don't store your searches, track your activity, or sell
					your data. Search freely knowing your privacy is protected.
				</p>
			</div>
		{/if}
		...
		{#if hasSearched}
			...
			<div class="space-y-6">
				<div
					class="flex items-center justify-between border-b border-purple-700/30 pb-4"
				>
					<div>
						<p class="text-purple-200">
							About <span class="font-medium text-white">
								847,000</span
							>
							results for
						...
						<p class="text-sm text-purple-400 mt-1">
							Searched privately in 0.31 seconds
						</p>
					</div>
					<div class="flex items-center gap-3">
						<Badge
						...
						<div class="relative export-menu">
							...
							{#if showExportMenu}
								<div
									class="absolute right-0 top-full mt-2 w-48 bg-purple-900/90 border border-purple-700/50 rounded-lg shadow-lg backdrop-blur-sm z-50"
								>
									<button
				...
				{:else}
											...
											<div class="flex-1 min-w-0">
												...
												>
													<a
														href={"https://" +
															result.url}
														class="text-sm text-purple-400 hover:text-purple-300 truncate"
													>
														{result.url}
													</a>
													{#if result.verified}
													...
													{/if}
													<span
														class="text-xs text-purple-400 ml-auto flex-shrink-0"
													>
														{result.timestamp}
													</span>
												</div>
												<h3
													class="text-xl font-semibold text-white hover:text-purple-400 cursor-pointer transition-colors mb-2"
												>
													{result.title}
												</h3>
												<p
													class="text-purple-200 leading-relaxed"
												>
													{result.description}
												</p>
											</div>
					...
					<div class="py-8">
						...
						<p class="text-center text-sm text-purple-400 mt-4">
							Page {currentPage} of {totalPages} • Showing {(currentPage -
								1) *
								resultsPerPage +
								1}-{Math.min(
		...
		<div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12">
			<div class="text-center">
				<div class="mb-4 flex items-center justify-center gap-2">
					<div
						class="flex h-6 w-6 items-center justify-center rounded-lg bg-gradient-to-br from-purple-500 to-purple-700"
					>
				...
				<p class="text-purple-200 mb-4">
					Privacy-first search engine. No tracking, no ads, no
					compromise.
				</p>
				<p class="text-sm text-purple-400">
					© 2025 Breeze Search. Built with privacy in mind.
				</p>
			</div>
		</div>
	</footer>
